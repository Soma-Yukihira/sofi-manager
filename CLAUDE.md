...
- **No release file, no SemVer tag, no manual `__version__` bump.** The
  git commit graph is still the version: `sofi_manager/version.py`
  reads `v<count> · <short sha> · <date>` from `git rev-list` / `git
  log` at runtime, displays it in the sidebar footer, logs it on `cli.py
  run`, and powers a one-shot "what's new" banner that links to the
  GitHub compare URL after a successful pull. Frozen `.exe` builds bake
  the same triple into `sofi_manager/_build_info.py` at build time
  (gitignored; generated by `tools/build.py`).
...
- **Don't reintroduce a release pipeline** (`tools/release.py`, a
  manually-bumped `__version__` constant, GitHub Releases poller)
  without an explicit request — the current owner has stated they don't
  want it. The git-derived version in `sofi_manager/version.py` is the
  agreed-upon middle ground: identifiable build, zero manual bump.
...
| `sofi_manager/version.py`                | Git-derived build identification (`v143 · sha · date`) |
